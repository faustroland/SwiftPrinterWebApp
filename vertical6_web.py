...
image_path = img_name
dir_path = get_directory_path(image_path)
...
with open(dir_path + "/image_hex.txt", "w",encoding="UTF-8") as text_file:
...
s = "\n".join(output_lines)
slen = len(s)
print("Lenght: " + str(slen) + "\nTable count:" + str(slen/(100000)) + "\nPrint time: " + str(len(output_lines)/((15 if ONLY_DOTS else 10)*3600)))
with open(dir_path+"/image_data.txt", "w",encoding="UTF-8") as text_file:
    text_file.write(s)
with open(dir_path+"/time.txt", "w",encoding="UTF-8") as time_file:
    time_file.write(str(ticks/30/3600))
